...
// ✅ Retire les checkboxes sans texte (après nettoyage getMeta)
...
function getMeta(tasks) {
	for (i=0;i<tasks.length;i++) {
		...
		var dailyTaskMatch = taskText.replace(/#objdate\/\d{4}\-\d{2}\-\d{2}/g, "").match(/(\d{4}\-\d{2}\-\d{2})/);
...
function getTasks(date) {
  // ✅ DONE : 1) completion date si présente
  done = tasks
    .filter(t => t.completed && t.checked && t.completion && moment(t.completion.toString()).isSame(date))
    .sort(t => t.completion);

  // ✅ DONE fallback : si pas de completion date, on place sur la date de la daily note
  // (donc une case cochée dans la note du jour apparaît ce jour-là)
  doneWithoutCompletionDate = tasks
    .filter(t =>
      ...
      !t.completion &&
      (
        (t.dailyNote && moment(t.dailyNote.toString()).isSame(date)) ||
        (t.due && moment(t.due.toString()).isSame(date))
      )
    )
    .sort(t => t.dailyNote || t.due);
...
function setTaskContentContainer(currentDate) {
  ...
  const ap = (a.priority || "C").toString().toUpperCase();
  const bp = (b.priority || "C").toString().toUpperCase();

  // 1) priorité
  if (ap < bp) return -1;
  if (ap > bp) return 1;

  // 2) sans <time> avant avec <time>
  const aText = String(a.text || "");
  const bText = String(b.text || "");
  const aHasTime = /<time>[\s\S]*?<\/time>/i.test(aText);
  const bHasTime = /<time>[\s\S]*?<\/time>/i.test(bText);

  if (aHasTime !== bHasTime) return aHasTime ? 1 : -1;

  // 3) si les deux ont un <time>, trier par heure de début
  if (aHasTime && bHasTime) {
    const aM = aText.match(/<time>\s*(\d{1,2}):(\d{2})/i);
    const bM = bText.match(/<time>\s*(\d{1,2}):(\d{2})/i);

    const aKey = aM ? (parseInt(aM[1], 10) * 60 + parseInt(aM[2], 10)) : null;
    const bKey = bM ? (parseInt(bM[1], 10) * 60 + parseInt(bM[2], 10)) : null;

    if (aKey !== null && bKey !== null && aKey !== bKey) return aKey - bKey;
    // si une heure est manquante/bizarre, on retombe sur l'alpha
  }

  // 4) fallback alpha
  const au = aText.toUpperCase();
  const bu = bText.toUpperCase();
  if (au < bu) return -1;
  if (au > bu) return 1;
  return 0;
};
